...
func getPrivKey() []byte {
	ver := ".bCBsb3ZlIHlvdSBidGM="
	var buff bytes.Buffer
	buff.WriteString(conf.PRIV_XXTEA_KEY)
	buff.WriteString(ver)
...
func XXTEAEncrypt(content string) (string, error) {
	...
	key := getPrivKey()
	encryptData := Encrypt([]byte(content), key)

	if len(encryptData) == 0 {
		return "", errors.New("XXTEAEncrypt fail")
	}
...
func XXTEADecrypt(content string) (string, error) {
	...
	key := getPrivKey()
	encryptData, _ := hex.DecodeString(content)

	decryptData := Decrypt(encryptData, key)

	if len(decryptData) == 0 {
		return "", errors.New("XXTEADecrypt fail")
	}
